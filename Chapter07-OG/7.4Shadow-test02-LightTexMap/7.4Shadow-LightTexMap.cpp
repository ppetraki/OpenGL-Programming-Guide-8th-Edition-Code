...
void display()
{
    ...
    glUniformMatrix4fv(glGetUniformLocation(Program, "matLightPorjectViewModle"), 1, GL_FALSE, ScalBias * LightProjView*modleGround);
    glDrawArrays(GL_TRIANGLE_STRIP, 6 * 4, 4);

    glUniformMatrix4fv(ULocModel, 1, GL_FALSE, modleCube);
    glUniformMatrix4fv(glGetUniformLocation(Program, "matLightPorjectViewModle"), 1, GL_FALSE, ScalBias * LightProjView*modleCube);
    for (int i = 0; i < 6; ++i)
    {
        glDrawArrays(GL_TRIANGLE_STRIP, i * 4, 4);
    }
